...
const LoginScreen = ({ onLogin }: LoginScreenProps) => {
  ...
  useEffect(() => {
    let mounted = true;
    (async () => {
      const masterExists = await hasMaster();
      if (!mounted) return;
      setHasExistingVault(masterExists);
      setIsSetup(!masterExists);
    })();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (lockStatus.locked) {
      setError(
        `App bloqueado. Tente novamente em ${lockStatus.secondsRemaining}s`,
      );
      return;
    }
    ...
    if (isSetup) {
      ...
      try {
        ...
        console.error("Erro ao criar cofre:", err);
        setError(
          "Erro ao criar cofre. Tente novamente. " +
      ...
      try {
        ...
        if (result.valid) {
          ...
          if (attempt.locked) {
            setError(
              `Muitas tentativas. Bloqueado por ${attempt.lockoutSeconds}s`,
            );
          } else {
            setError(
              `Senha incorreta. ${attempt.remainingAttempts} tentativas restantes`,
            );
          }
        }
      } catch (err) {
      ...
      <form onSubmit={handleSubmit} className="w-full space-y-4">
        ...
        {isSetup && (
          <div className="text-xs text-muted-foreground space-y-0.5">
            ...
            <p className={/[!@#$%&*]/.test(password) ? "text-success" : ""}>
              • Um símbolo (!@#$%&*)
            </p>
          </div>
        )}
